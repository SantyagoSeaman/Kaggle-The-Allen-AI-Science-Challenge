...
try:
    ...
    for index, row in training_set[0:100].iterrows():
        ...
        for minimum_should_match in range(100, 60, -5):
            res = es.search(
                body={"query":
                  {"match":
                    {'text': {
        ...
        for answer in row[['answerA', 'answerB', 'answerC', 'answerD']]:
            ...
            # Search documents
            for fuzziness in [0, 1, 2]:
                res = es.search(
                    index="ai_wiki_full", size=20,
                    body={"query": { "filtered": {
                        "query": {
                            "match_all": {}
                        },
            ...
            # Just arrange founded documents by score
            if res['hits']['total'] > 0:
                ids = []
                for hit in res['hits']['hits']:
                    source = hit["_source"]
                ...
                # take into consideration categories matching
                res = es.search(
                    index="ai_wiki_full",
                    body={"query":
                          {"match":
