...
try:
    ...
    # for index, row in training_set[start_index:(start_index + 200)].iterrows():
        ...
        knowledge_domain = [x[0] for x in categories_freq[0:50]]
        ...
        for fuzziness in [1, 2, 3, 4, 5]:
            ...
            # if we have documents at least for 2 answers, we exit
   ...
   sample_submission_score.to_csv('data/impl_5_' + datetime.datetime.now().strftime("%Y-%m-%d-%H-%M-%S") + '.csv', index=False)
